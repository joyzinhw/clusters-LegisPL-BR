...
def obter_data_status_proposicao(id_proposicao):
    url = f"{BASE_URL}/proposicoes/{id_proposicao}"
    try:
        resp = requests.get(url, headers={"accept": "application/json"})
        if resp.status_code != 200:
            return "Erro"
        dados = resp.json()["dados"]
        data_string = dados.get("statusProposicao", {}).get("dataHora")
        if data_string:
            data_obj = datetime.fromisoformat(data_string)
...
def obter_ementa_detalhada(id_proposicao):
    url = f"{BASE_URL}/proposicoes/{id_proposicao}"
    try:
        resp = requests.get(url, headers={"accept": "application/json"})
        if resp.status_code != 200:
            return "Erro"
        dados = resp.json()["dados"]
        return dados.get("ementaDetalhada", {})
    except Exception as e:
        return f"Erro: {e}"
...
def main():
    ...
    for ano in anos:
        ...
        for prop in proposicoes:
            ...
            prop["data_status"] = obter_data_status_proposicao(prop["id"])
            prop["ementa detalhada"] = obter_ementa_detalhada(prop["id"])
