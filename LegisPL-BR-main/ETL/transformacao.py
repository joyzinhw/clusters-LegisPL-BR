...
def foi_finalizado(status):
    finais = [
        ...
        "retirada", "declarada prejudicada", "encerrada", "vetado totalmente",
        "devolvida ao(à) Autor(a)", "retirado pelo(a) Autor(a)", "transformado em nova proposição",
        "aguardando sanção"
    ]
    if pd.isna(status):
        return False
...
def foi_aprovado(status):
    termos = ["transformada em norma", "transformado em norma jurídica", "transformado em lei", "promulgado", "sanção", "aguardando sanção"]
    if pd.isna(status):
        return False
...
df = pd.read_excel("projetos_PL_2021_2024_completo.xlsx")

# Converte a coluna 'data da apresentação' para o tipo datetime
df['data da apresentação'] = pd.to_datetime(df['data da apresentação'])

df['ano_eleitoral'] = df['data da apresentação'].dt.year.apply(lambda x: 1 if x == 2022 else 0)

df['governo'] = df['data da apresentação'].dt.year.apply(lambda x: 'direita' if x in [2021, 2022] else 'esquerda')
...
df.to_excel("projetos_PL_2021_2024_transformado.xlsx", index=False)
print("Arquivo salvo como 'projetos_PL_2021_2024_transformado.xlsx'")
'''

df = pd.read_excel("projetos_PL_2021_2024_transformado.xlsx")
df_composicao = pd.read_excel("analisados.xlsx")

df_composicao['data'] = df_composicao['data'].dt.strftime('%m%Y')
print(df['data_status'][0])
print(df_composicao['data'][0])

df_propostas_com_composicao = pd.merge(
    df,
    df_composicao[['data', 'qtd_esquerda', 'qtd_direita', 'qtd_centrao']],
    left_on='data_status',
    right_on='data',
    how='left'
)

df_propostas_com_composicao.to_excel("projetos_PL_2021_2024_transformado_com_composição.xlsx", index=False)
'''