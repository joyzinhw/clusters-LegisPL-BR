...
#df = pd.read_excel("../projetos_PL_2021_2024_transformado_com_composição.xlsx")
print(len(df))
df = df[~df['autores'].str.startswith('Senado Federal', na=False)]
print(len(df))
...
print(len(df)) #580
colunas = ['ano', 'data_status', 'região', 'dias_tramitacao', 'tema_dominante', 'bloco_partidario', 'autoria_coletiva', 'aprovado', 'ano_eleitoral', 'governo']
#colunas = ['ano', 'região', 'dias_tramitacao', 'tema_dominante', 'bloco_partidario', 'autoria_coletiva', 'aprovado', 'ano_eleitoral', 'governo', 'qtd_esquerda', 'qtd_direita', 'qtd_centrao']
df = df[colunas]
